...
/**
 * The hybrid search experience (client island on /products).
 *
 * Slayed products render as SlayCards linking to their hierarchical slay
 * pages. External matches land in "Not Yet Slayed" with an instant Score
 * Engine preview and a full-slay request (queued as pending-review).
 */

import { useState } from "react";
import Link from "next/link";
import { SlayCard } from "./slay-card";
...
export function ProductSearch() {
  ...
  return (
    <div>
      ...
      {actionError && <p className="mt-4 text-sm text-ember">{actionError}</p>}
      ...
      {slayed.length > 0 && (
        <div className="mt-10">
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {slayed.map((p) => (
              <Link key={p.slug} href={productPath(p)} className="block">
                <SlayCard
                  name={p.name}
